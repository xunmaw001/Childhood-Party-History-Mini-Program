<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="党史分类" prop="dangshifenlei">
          <el-input v-model="ruleForm.dangshifenlei" 
              placeholder="党史分类" clearable  :readonly="ro.dangshifenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	dangshifenlei : false,
      ...
      rules: {
          dangshifenlei: [
                { required: true, message: '党史分类不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangshifenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangshifenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
