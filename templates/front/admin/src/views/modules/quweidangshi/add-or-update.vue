<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="年龄段" prop="nianlingduan">
        ...
        <div v-else>
          <el-form-item class="input" label="年龄段" prop="nianlingduan">
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="党史介绍" prop="dangshijieshao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.dangshijieshao" label="党史介绍" prop="dangshijieshao">
                    <span v-html="ruleForm.dangshijieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/nianlingduan/nianlingduan`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.nianlingduanOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.dangshijieshao = this.ruleForm.dangshijieshao.replace(reg,'../../../django4xa3d/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `quweidangshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.quweidangshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
