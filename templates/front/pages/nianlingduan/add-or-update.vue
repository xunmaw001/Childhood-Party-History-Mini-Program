	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`nianlingduan`, this.ruleForm.id);
...
//跨表计算判断
				if((!this.ruleForm.nianlingduan)){
					this.$utils.msg(`年龄段不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`nianlingduan`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`nianlingduan`, this.ruleForm);
						}else{
							await this.$api.add(`nianlingduan`, this.ruleForm);
						}
