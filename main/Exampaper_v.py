...
def exampaper_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =exampaper.page(exampaper, exampaper, req_dict, request)
...
def exampaper_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if req_dict.get("mima") and req_dict.get("password"):
            if "mima" not  in exampaper.getallcolumn(exampaper,exampaper) :
                del req_dict["mima"]
            if  "password" not  in exampaper.getallcolumn(exampaper,exampaper) :
...
def exampaper_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def exampaper_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'exampaper' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from exampaper where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from exampaper where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def exampaper_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM exampaper {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
